...
class LaneLines():
    ...
    # function for detecting the lane pixels
    def find_lane_pixels(self):
        ...
        # this parameter specifies if the lanelines are detected or not from the specified parameters
        self.detected = True

        # from here on, we will code for including data from previous detected lanelines
        if (leftx == 0 and rightx == 0):
            self.detected = False
        if (lefty == 0 and righty == 0):
            self.detected = False

        cur_left_fit = self.left_fit
        cur_right_fit = self.right_fit
        ...
        if (self.detected == False):
            cur_left_fit = np.polyfit(lefty, leftx, 2)
            cur_right_fit = np.polyfit(righty, rightx, 2)

        return leftx, lefty, rightx, righty, out_img


    # function to fit a polynomial to our detected pixels
